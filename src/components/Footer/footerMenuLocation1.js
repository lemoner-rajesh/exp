...
const FooterMenuLocation1 = (props) => {

const menuqry = useStaticQuery(graphql`
{
    menuEn:  wpMenu(name: {eq: "Footer Menu 1 EN"}) {
        menuItems {
          nodes {
            label
            parentId
    ...
    menuAr: wpMenu(name: {eq: "Footer Menu 1 AR"}) {
      menuItems {
        nodes {
          label
          parentId
      ...
      menuTr:  wpMenu(name: {eq: "Footer Menu 1 TR"}) {
        menuItems {
          nodes {
            label
            parentId
...
export default FooterMenuLocation1
